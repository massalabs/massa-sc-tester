...
impl Ledger {
    ...
    pub(crate) fn sub(&mut self, address: &Address, amount: u64) -> Result<()> {
        ...
        if entry.balance.checked_sub(amount).is_none() {
            bail!("Fail to set balance substraction for {} of amount {} in the ledger", address, amount)
        }
...
impl InterfaceImpl {
    pub(crate) fn new() -> Result<InterfaceImpl> {
        let file = std::fs::File::open("./ledger.json")?;
        let reader = std::io::BufReader::new(file);
        Ok(InterfaceImpl {
            ledger: serde_json::from_reader(reader)?,
    ...
    pub(crate) fn set_data_entry(&self, address: &Address, key: &str, value: Bytecode) -> Result<()> {
        match self.ledger.lock() {
            Ok(ledger) => {
